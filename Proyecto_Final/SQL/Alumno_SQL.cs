    ...
    internal class Alumno_SQL
    {
        ...
        public DataTable Buscar_Horario(Alumnos alumnos)
        {
            ...
            using (SqlConnection conexion = Conexion.Conectar())
            {
                ...
                string sentencia = "select Grupo,Materia,Salon,Profesor,LunesI,LunesF,MartesI,MartesF,MiercolesI,MiercolesF,JuevesI,JuevesF,ViernesI,ViernesF from Clases where Turno = @Turno and Semestre = @Semestre " + alumnos.Sentencia;
                try
                {
                    cmdSelect = new SqlCommand(sentencia, conexion);
                    cmdSelect.Parameters.AddWithValue("@Turno", alumnos.Turno);
